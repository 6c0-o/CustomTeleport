...
public class DisableCommand implements CommandExecutor, TabCompleter {
    ...
    public DisableCommand(CustomTeleport plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length != 2) {
            MessageUtil.sendMessage(player, "&eUsage: /ctdisable <type> <world>");
            return true;
        }
        ...
        worldSection.set("disable", true);

        plugin.saveConfig();
        MessageUtil.sendMessage(player, "&e" + world + " " + type +" command is now disable.");
        return true;
    }
